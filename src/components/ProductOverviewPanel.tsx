...
export function ProductOverviewPanel({ data, accounts, registry = [], view, onView, onUpdated, error }: { data: ProductOverview | null; accounts: ProviderUsage[]; registry?: RegistryAccount[]; view: View; onView: (view: View) => void; onUpdated?: () => void | Promise<void>; error?: string }) {
  ...
  return <>
    ...
    {view === 'details' ? <section className="product-summary" aria-label="Subscription overview">
      ...
      <button className="summary-answer" onClick={() => onView('usage')}><span>If paid by API · {month}</span><strong>{data.usage.tokens === null ? (confirmedSubset && api !== null ? `≥ ${fmtMoney(api)}` : 'Usage unavailable') : api === null ? 'Pricing incomplete' : `${partialApi ? '≥ ' : ''}${fmtMoney(api)}`}</strong><small>{confirmedSubset ? `Confirmed subset · ≥ ${fmtTokens(confirmedTokens ?? 0)} tokens; native observations may overlap` : data.usage.reconciliation?.status === 'partial' ? 'Native observations may overlap; see confirmed subtotal' : data.usage.tokens === null ? 'Monthly usage has not been imported' : partialApi ? 'Known prices; some models unpriced' : `${fmtTokens(data.usage.tokens)} tokens measured`} →</small></button>
    ...
    {view === 'overview' || view === 'accounts' ? <section className="product-panel"><div className="section-heading"><div><h2>Other subscriptions & accounts</h2><p>Live CLIProxyAPI credentials and API observations. Declared accounts without an automatic source are listed below.</p></div>{view === 'overview' ? <button className="small-button" onClick={() => onView('accounts')}>All {registry.length} accounts →</button> : null}</div><div className="quota-strip">
      {shownAccounts.map(account => {
        ...
        return <article className="quota-mini" key={account.id}><span className="provider-identity"><ProviderIcon provider={account.provider} /><span><small>{account.provider}</small>{account.label}</span></span>{account.funds ? <>
          <strong>{account.funds.accountBalance.usd === null ? 'Account balance unknown' : `${fmtMoney(account.funds.accountBalance.usd)} account balance`}</strong>
          ...
          <strong>CLIProxyAPI · {account.proxyCredential.status}</strong>
        ...
        </> : <><strong>{fresh && account.quota.remaining !== null ? account.quota.remaining === 0 ? 'Exhausted' : `${account.quota.remaining}${account.quota.unit === 'percent' ? '%' : ''} left` : 'Quota unknown'}</strong><small>{account.quota.status === 'error' ? 'Automatic source failed; other sources continue' : account.quota.status === 'stale' ? 'Source observation is stale' : fresh ? 'Recent provider observation' : 'No current automatic quota observation'}</small></>}<small>{account.funds ? account.proxyConfigured ? 'CLIProxyAPI account linked' : 'CLIProxyAPI account association not observed' : account.proxyCredential ? account.proxyCredential.kind === 'oauth' ? 'CLIProxyAPI OAuth credential' : 'CLIProxyAPI upstream API key' : 'Direct account · not a CLIProxyAPI credential'}</small>{Number.isFinite(observed) ? <small>Observed {fmtDate(account.quota.observedAt!,'Asia/Jerusalem')}</small> : null}{account.operatorNote ? <small>Operator note: {account.operatorNote}</small> : null}{website ? <a className="action-link" href={website} target="_blank" rel="noreferrer" title="Provider website in your current browser; verify the signed-in account">Provider website ↗</a> : null}</article>;
      })}
    ...
    {view === 'overview' || view === 'usage' ? <section className="product-panel"><div className="section-heading"><div><h2>Who uses the most?</h2><p>{month} · {data.usage.tokens === null ? (confirmedSubset ? `≥ ${fmtTokens(confirmedTokens ?? 0)} tokens · ≥ ${data.usage.reconciliation?.confirmedRequests?.toLocaleString() ?? 'Unknown'} requests · confirmed subset` : 'Monthly usage unavailable') : `${fmtTokens(data.usage.tokens)} tokens · ${data.usage.requests?.toLocaleString() ?? 'Unknown'} requests`}</p></div>{view === 'overview' ? <button className="small-button" onClick={() => onView('usage')}>Usage details →</button> : null}</div>
