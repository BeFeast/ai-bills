...
export function ProductOverviewPanel({ data, accounts, registry = [], view, onView, onUpdated, error }: { data: ProductOverview | null; accounts: ProviderUsage[]; registry?: RegistryAccount[]; view: View; onView: (view: View) => void; onUpdated?: () => void | Promise<void>; error?: string }) {
  ...
  const shownAccounts = view === 'overview' ? otherAccounts.filter(account => /meta|muse|kimi|xai|x.ai|cursor|ollama|openrouter/i.test(account.provider)) : otherAccounts;
  return <>
    ...
    {view === 'overview' || view === 'accounts' ? <section className="product-panel"><div className="section-heading"><div><h2>Other subscriptions & accounts</h2><p>Quota unknown until an automatic source is connected.</p></div>{view === 'overview' ? <button className="small-button" onClick={() => onView('accounts')}>All {registry.length} accounts →</button> : null}</div><div className="quota-strip">
      {shownAccounts.map(account => {
        ...
        </> : <><strong>{fresh && account.quota.remaining !== null ? account.quota.remaining === 0 ? 'Exhausted' : `${account.quota.remaining}${account.quota.unit === 'percent' ? '%' : ''} left` : 'Quota unknown'}</strong><small>{account.quota.status === 'error' ? 'Automatic source failed; other sources continue' : account.quota.status === 'stale' ? 'Source observation is stale' : fresh ? 'Recent provider observation' : 'No current automatic quota observation'}</small></>}<small>{account.funds ? account.proxyConfigured ? 'CLIProxyAPI account linked' : 'CLIProxyAPI account association not observed' : account.routingEnrolled === true ? 'Proxy routing linked' : account.routingEnrolled === false ? 'Independent account · not enrolled in proxy routing' : 'Proxy linkage unknown'}</small>{Number.isFinite(observed) ? <small>Observed {fmtDate(account.quota.observedAt!,'Asia/Jerusalem')}</small> : null}{account.operatorNote ? <small>Operator note: {account.operatorNote}</small> : null}{website ? <a className="action-link" href={website} target="_blank" rel="noreferrer" title="Provider website in your current browser; verify the signed-in account">Provider website ↗</a> : null}</article>;
      })}
