  ...
  return <section className="billing" aria-label="Routing controls">
    ...
    {state ? <>
      ...
      <div className="bill-grid mini">
        ...
        <Metric tone="live" label="Available allowance" value={microMoney(remainingAllowance(state.budget))} note="Shared across clients, accounts and retries" />
      </div>
      <div className="routing-budget-bar" role="img" aria-label={`${microMoney(state.budget.spent_microusd)} settled and ${microMoney(state.budget.reserved_microusd)} reserved of ${microMoney(state.budget.limit_microusd)}`}><span className="settled" style={{ width: `${Math.min(100, state.budget.spent_microusd / Math.max(1, state.budget.limit_microusd) * 100)}%` }} /><span className="reserved" style={{ width: `${Math.min(100, state.budget.reserved_microusd / Math.max(1, state.budget.limit_microusd) * 100)}%` }} /></div>
