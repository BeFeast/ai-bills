...
export type LimitsHero = { refreshedAt: string | null; windowHours: number; recencyKnown: boolean; loading: boolean; cards: LimitsHeroCard[] };
...
/** The usage service seeds a placeholder per account until the first observation lands; that is not a failing source. */
const isPending = (result: ProviderUsage) => !result.ok && result.status === undefined && /first quota observation/i.test(result.error ?? '');
...
export function buildLimitsHero({ usage, registry, last24h, now }: { usage: ProviderUsage[]; registry: RegistryAccount[]; last24h?: OverviewRecentUsage; now: number }): LimitsHero {
  ...
  const observed = usage.filter((result) => !isPending(result));
  ...
  return { refreshedAt: last24h?.observedAt ?? latestObservation, windowHours: last24h?.windowHours ?? 24, recencyKnown, loading: usage.length === 0 || observed.length === 0, cards: [...quota, ...errors, ...outcomes] };
}
