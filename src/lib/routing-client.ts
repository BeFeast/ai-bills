...
export type RoutingModel = { id: string; label: string; status: 'approved' | 'hidden' | 'denied'; tool_call?: boolean; capabilities: string[]; input_limit_tokens: number | null; output_limit_tokens: number | null; prices: Record<string, number | null> | null; routes: { account_id: string; billing: 'included' | 'paid'; upstream_model: string; upstream_canonical_model?: string; allowed_clients?: string[]; prices?: Record<string, number | null> | null; price_version?: string; price_evidence?: string }[] };
