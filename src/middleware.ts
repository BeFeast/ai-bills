...
const withClerk = clerkMiddleware(async (auth, request) => {
  ...
  // API routes only — pages always go through the session — and the Node side resolves the token through
  // ingest_tokens, answering 401 itself when it is not a live one (every API route calls requireTenant first).
  if (membershipMode() && isApi(request) && /^Bearer\s+\S+$/i.test(request.headers.get('authorization') ?? '')) return NextResponse.next({ request: { headers } });
